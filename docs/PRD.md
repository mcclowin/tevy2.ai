...
### Provisioning Speed (current: ~10min, target: ~3min)

Current cold boot is slow due to sequential cloud-init work and OpenClaw gateway cold start on CX23 (2 vCPU, 4GB RAM).

**Quick wins:**

| Optimization | Estimated saving | Effort |
|---|---|---|
| Pre-warm OpenClaw in snapshot (run gateway once during snapshot creation so Node JIT cache is warm) | -2min | Low — add to snapshot build script |
| Remove binary verification step (snapshot already has working openclaw, skip "is binary 0 bytes" check) | -1min | Low — delete 5 lines from cloud-init |
| Pre-clone tevy-agent-image repo in snapshot (`/opt/tevy` baked in, cloud-init just does `git pull`) | -30s | Low — add to snapshot build script |
| Don't wait for gateway in cloud-init (write configs, start service, exit — let dashboard poll `boot-status`) | -1min | Low — remove wait loop from cloud-init |
| Parallelize config writes (write all base64 files in parallel, not sequential) | -10s | Low — use `&` + `wait` in bash |

**Medium-term:**

| Optimization | Estimated saving | Effort |
|---|---|---|
| Upgrade to CX33 (4 vCPU, 8GB) for faster cold start | -1-2min | Config change — higher cost (€7.49/mo) |
| Hetzner server pool — pre-provision 2-3 warm standby VPSes, assign on signup | -8min (near-instant) | Medium — need pool management + cost of idle servers |
| Lighter OpenClaw profile — disable unused plugins/features at boot | -30s | Medium — needs OpenClaw config tuning |
| Snapshot per region — create snapshots in multiple DCs to avoid cross-DC image transfer | -20s | Low — run snapshot build in each location |

**Stretch:**

| Optimization | Impact | Effort |
|---|---|---|
| Container-per-customer on shared host (Docker/Podman) instead of 1:1 VPS | Sub-30s deploy, lower cost | High — rearchitecture, security isolation concerns |
| OpenClaw fast-start mode (if upstream adds it) | Depends on upstream | None — wait for upstream |

### Content Publishing Automation

- Auto-schedule and publish approved content to connected platforms
- Integration with Buffer/Hootsuite APIs as fallback for platforms without direct browser support
- Approval queue: agent drafts → customer reviews in dashboard → one-click publish
- A/B testing: publish variant copies and track performance

### Advanced Analytics

- Pull engagement metrics from connected platforms into the dashboard
- Weekly performance reports generated by the agent
- Attribution tracking: which TevY2-generated content drove leads/conversions
- Benchmark against competitor metrics collected via browser intelligence

### Multi-Agent Support

- Allow customers to spin up specialized sub-agents (e.g., one for content, one for research)
- Shared workspace across agents for brand consistency
- Per-agent billing and resource limits

### White-Label / Agency Mode

- Agency customers manage multiple client agents from a single dashboard
- Custom branding on the dashboard per agency
- Aggregated billing and reporting across all client agents
- Client-facing read-only dashboard view

---

*Created: 2026-03-04 | Last updated: 2026-03-20 22:34 UTC*
